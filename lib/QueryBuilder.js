(function() {
    ...
    module.exports = new Class({
        ...
        , in: function(command) {
            ...
            if (type.function(values.isQuery)) {
                ...
                if (values.length) {
                    ...
                    if (uniqueValues.some(v => typeof v !== 'number' || Number.isNaN(v))) {
                        return ' IN ('+uniqueValues.map(value => this.renderInValueValue(value)).join(', ')+')';
                    } else {
        ...
        /**
         * render null & notNull values
         */
        , renderInValueValue: function(value) {
            if (type.function(value)) {
                const data = value();

                if (data.fn === 'null') return 'null';
                else return this.escape(`cannot process function value that is other than null`);
